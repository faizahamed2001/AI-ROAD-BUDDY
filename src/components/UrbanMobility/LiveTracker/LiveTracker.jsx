...
function parseCSV(csv, type) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Skip if row is a duplicate header row (all columns match headers, or matches known alternate header forms)
    if (row.length < headers.length) continue;
    const isHeaderRow = row.every((cell, idx) => cell === headers[idx]) ||
      row.join(',').toLowerCase().includes('bus no.') ||
      row.join(',').toLowerCase().includes('train no.');
  ...
  console.log(`Parsed ${data.length} ${type} records`);
  return data;
}
...
useEffect(() => {
  ...
  if (src) {
    filteredData = filteredData.filter(row =>
      (row['From'] || '').toLowerCase().includes(src)
    );

    if (dst) {
      filteredData = filteredData.filter(row =>
        (row['To'] || '').toLowerCase().includes(dst)
      );
    }
  } else if (dst) {
    // If only destination is provided, filter by 'To' column
    filteredData = filteredData.filter(row =>
      (row['To'] || '').toLowerCase().includes(dst)
    );
  }
